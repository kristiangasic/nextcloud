...
#              Bereitgestellt von Gasic.bio                #
#                 Lizenz zur freien Verwendung             #
#                Bei Fragen: kristian@gasic.bio            #
#                                                          #
############################################################
...
install_nextcloud() {
    ...
    sudo apt install php8.3 libapache2-mod-php8.3 php8.3-gd php8.3-mysql php8.3-curl php8.3-mbstring php8.3-intl php8.3-imagick php8.3-xml php8.3-zip php8.3-opcache php8.3-redis redis-server -y || { echo "Failed to install PHP packages"; exit 1; }
...
EOF
    ...
    if ! mysql -u${MARIADB_USER} -p${MARIADB_PASSWORD} -e "SHOW DATABASES;" > /dev/null 2>&1; then
        echo "Failed to connect to MariaDB. Please check your credentials."
        exit 1
    fi
...
EOF'
    ...
    NEXTCLOUD_URL=$(curl -s https://nextcloud.com/install | grep -oP '(?<=href=")[^"]+server/releases/latest.zip')
    wget "$NEXTCLOUD_URL" -O latest.zip || { echo "Failed to download Nextcloud"; exit 1; }
...
install_ssl() {
    ...
    echo "SSL installation complete. Automatic renewal is enabled."
}
